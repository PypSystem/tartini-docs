...
WABA connects your business number through Meta's official API. It's more stable than the QR Code connection and lets you send approved message templates.
...
If you already have a number ready on WABA, skip to the next section. If not, choose a path:

**Direct registration with Meta**: during the connection itself in Tartini, Meta offers a simplified registration to sign up a new number.

**Authorized partner (BSP)**: a company authorized by Meta handles provisioning, porting, and support, usually with payment in reais and an invoice in Brazil. It's the common path for porting a number you already use.

<Tip>
  Meta doesn't sell numbers directly. Without a BSP, direct registration requires you to already have the Meta Business account and the WABA ready.
</Tip>
...
<AccordionGroup>
  ...
  Yes. First unlink the number from regular WhatsApp Business, then register it on WABA through Meta or through a BSP.
  </Accordion>
  <Accordion title="What's the difference from regular WhatsApp Business?">
  WABA is Meta's official API: it doesn't carry ban risk from an unauthorized connection and lets you send approved templates. WhatsApp Business by QR Code uses an unofficial connection.
  </Accordion>
...
<CardGroup cols={2}>
  <Card title="Channels explained" href="/en/canais/canais-explicados">The channels available in Tartini and when to use each one.</Card>
  <Card title="Create WhatsApp message templates" href="/en/canais/templates">Messages approved by Meta to start conversations.</Card>
  <Card title="Fix connection issues" href="/en/canais/problemas-de-conexao">Common symptoms and how to fix them.</Card>
  <Card title="Turn on Tino for a channel" href="/en/tino/ia-por-canal">Set up hours and follow-up for this number.</Card>
</CardGroup>
