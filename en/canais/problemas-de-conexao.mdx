...
## You see "Já existe um canal" or "Esta entidade já possui o canal"

These messages mean "A channel already exists" and "This entity already has the channel".

**Cause:** the Unit you chose already has a channel of that type, even if it's disconnected. QR Code and WABA count as the same WhatsApp channel.

**Fix:** reconnect the channel that already exists on that Unit, or connect the number on a different Unit. To switch from QR Code to WABA, or the other way around, delete the current channel first.
