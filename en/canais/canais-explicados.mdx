...
Tartini connects five types of channel:
...
- **Telegram**: Telegram's official connection, created in @BotFather.
...
Telegram connects through Telegram's official API, with a token generated in @BotFather. Your contacts always start the conversation, through the link or the QR Code you share.

The customer is identified by their Telegram account, not by phone number. See the details in [Connect Telegram](/en/canais/telegram).
...
Each Unit can have one WhatsApp channel, by QR Code or by WABA, plus one Chat Widget and one Telegram channel. To connect another number, use another Unit.
...
<AccordionGroup>
  ...
  No, but the Unit has one WhatsApp channel at a time. To switch, delete the current channel and connect the other type; just disconnecting doesn't free the slot.
  </Accordion>
</AccordionGroup>
...
<Lista>
  ...
  <Artigo titulo="Connect Telegram" href="/en/canais/telegram">Handle Telegram conversations in the same Inbox.</Artigo>
