...
Each Unit can have one WhatsApp Business (QR Code) channel, one WABA channel, one Chat Widget, and one Telegram channel, each connected only once. To connect another number of the same type, use another Unit.
