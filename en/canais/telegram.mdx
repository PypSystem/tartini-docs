...
### The channel stopped receiving messages after changes in @BotFather

**Cause:** the token was revoked or changed in @BotFather. The old token stops working and the channel stops. {/* CONFIRMAR: se o status do canal muda sozinho para Desconectado quando o token é revogado */}

**Fix:** copy the current token in @BotFather, click the ⋮ next to the channel, and paste the token under **Trocar bot** (Change bot) or **Conectar bot** (Connect bot). With the same bot, the published link stays the same.
