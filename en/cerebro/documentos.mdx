...
Uploading a document is the fastest way to put existing content into the knowledge base: Tino reads the file and learns on its own, without you needing to rewrite anything.
...
<Steps>
  ...
  <Step title="Choose the audience">Check **Conhecimento Público** (Public Knowledge, used by Tino) or **Conhecimento Privado** (Private Knowledge, used only by Copilot).</Step>
...
If a file exceeds the size limit or isn't in an accepted format, it's rejected before uploading, and the rest of the batch's files proceed normally.
...
When indexing finishes, the content can already be used by Tino (if public) or by Copilot (if private or public). A **Tino aprendeu** (Tino learned) card may appear with a summary of what was extracted, and a button to test the knowledge in the Cérebro's side chat.
...
<AccordionGroup>
  ...
  Only if the chosen audience includes Conhecimento Público. With Conhecimento Privado, the content stays restricted to Copilot and the internal team.
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="Use the knowledge base intelligence" href="/en/cerebro/inteligencia-da-base">Review what Tino discovered from your documents.</Card>
</CardGroup>
