...
<Note>
  Who can see this: only those with access granted by a manager, under **Configurações > Empresa > Usuários** (Settings > Company > Users) (Sotto column). Active Sotto, which suggests on its own without you asking, is available on every plan.
...
<Note>
  Active Sotto warns you when it detects a possible inconsistency between your reply and the knowledge base.
</Note>
...
<AccordionGroup>
  ...
  It only appears on conversations handled by an agent and assigned to you.
  </Accordion>
