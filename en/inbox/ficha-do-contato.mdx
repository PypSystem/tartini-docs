...
The **Conversas** (Conversations) card lists up to the 50 most recent conversations, with status, channel, department, agent, and the link to the Sotto evaluation. The **Vínculos** (Links) card shows contacts with the same phone number on another channel (merge candidates) and this same contact's record in other units.
...
<Lista>
  ...
  <Artigo titulo="Read Sotto's evaluations" href="/en/sotto/avaliacoes">The summary and score for each conversation.</Artigo>
</Lista>
