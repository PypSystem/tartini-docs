...
A **Conexão via API** transforma o Tartini em um canal acessível pelos seus próprios sistemas: você consegue enviar e receber mensagens, criar contatos, iniciar conversas e consultar dados via REST. A página gerencia os **tokens de API** que autenticam essas chamadas.
...
- **Um token por integração**: se três sistemas conversam com o Tartini, crie três tokens. Assim, ao desativar um sistema, você revoga só o token dele.
